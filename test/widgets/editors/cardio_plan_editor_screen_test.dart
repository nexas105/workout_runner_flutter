...
Widget _wrap(Widget child) => MaterialApp(
  home: WorkoutRunnerTheme(data: WorkoutRunnerThemeData.dark(), child: child),
);

CardioPlan _samplePlan() =>
    CardioPlanBuilder(
          '5x400m intervals',
        ...
        .interval(name: 'Sprint', duration: const Duration(minutes: 2), met: 11)
        .rest(duration: const Duration(minutes: 1))
        .cooldown(duration: const Duration(minutes: 5))
        .build();

void main() {
  testWidgets('renders empty form when existing is null and Save is disabled', (
    tester,
  ) async {
    await tester.pumpWidget(_wrap(CardioPlanEditorScreen(onSave: (_) {})));
  ...
  testWidgets('pre-fills from existing plan: intervals count and discipline', (
    tester,
  ) async {
    final plan = _samplePlan();
    await tester.pumpWidget(
      _wrap(CardioPlanEditorScreen(existing: plan, onSave: (_) {})),
    );
  ...
  testWidgets('adding an interval increments the row count', (tester) async {
    await tester.pumpWidget(
      _wrap(CardioPlanEditorScreen(existing: _samplePlan(), onSave: (_) {})),
    );
        ...
        find
            .byWidgetPredicate(
              (w) =>
                  ...
                  ((w.key as ValueKey).value as String).startsWith(
                    'cardio-plan-interval-',
                  ),
            )
            .evaluate()
            .length;
  ...
  testWidgets('removing the last interval disables Save', (tester) async {
    ...
    await tester.tap(find.byKey(const ValueKey('cardio-plan-interval-kebab')));
  ...
  testWidgets('Save fires onSave with the constructed plan', (tester) async {
    ...
    await tester.pumpWidget(
      _wrap(
        CardioPlanEditorScreen(existing: plan, onSave: (p) => captured = p),
      ),
    );
