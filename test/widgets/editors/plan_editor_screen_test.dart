...
WorkoutPlan _samplePlan() =>
    WorkoutPlanBuilder('Push day', id: 'push-day')
...
void main() {
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.pumpWidget(
        _wrap(PlanEditorScreen(existing: null, onSave: (plan) => saved = plan)),
      );
      ...
      expect(
        find.byKey(const Key('plan_editor_validation_badge')),
        findsOneWidget,
      );
  ...
  testWidgets(
    'PlanEditorScreen pre-fills name and exercises from existing plan',
    (tester) async {
      await tester.pumpWidget(
        _wrap(PlanEditorScreen(existing: _samplePlan(), onSave: (_) {})),
      );
  ...
  testWidgets('Reordering exercises is reflected in the saved plan', (
    ...
    final reorderableFinder = find.byKey(
      const Key('plan_editor_exercise_list'),
    );
