...
void main() {
  ...
  test('recent returns newest-first by ratedAt', () async {
    await storage.save(
      SessionRating(sessionKey: 'old', ratedAt: DateTime.utc(2026, 1, 1)),
    );
    await storage.save(
      SessionRating(sessionKey: 'mid', ratedAt: DateTime.utc(2026, 3, 1)),
    );
    await storage.save(
      SessionRating(sessionKey: 'new', ratedAt: DateTime.utc(2026, 5, 1)),
    );
  ...
  test('recent respects limit', () async {
    for (var i = 0; i < 5; i++) {
      await storage.save(
        SessionRating(sessionKey: 'k$i', ratedAt: DateTime.utc(2026, 5, i + 1)),
      );
    }
  ...
  test('recent filters by since (inclusive)', () async {
    await storage.save(
      SessionRating(sessionKey: 'jan', ratedAt: DateTime.utc(2026, 1, 15)),
    );
    await storage.save(
      SessionRating(sessionKey: 'mar', ratedAt: DateTime.utc(2026, 3, 10)),
    );
    await storage.save(
      SessionRating(sessionKey: 'may', ratedAt: DateTime.utc(2026, 5, 1)),
    );
  ...
  test('remove deletes only the matching key', () async {
    await storage.save(
      SessionRating(sessionKey: 'a', ratedAt: DateTime.utc(2026, 5, 1)),
    );
    await storage.save(
      SessionRating(sessionKey: 'b', ratedAt: DateTime.utc(2026, 5, 2)),
    );
